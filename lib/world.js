...
exports.World = function (io) {
	...
	var world = {
		...
		action: function(x, y, user) {
			// Chop tree
			if(this.boardAt(x,y) === world.tiles.tree) {
				this.setBoardAt(x,y,world.tiles.treeChopped);
				return { a: true, type: world.tiles.treeChopped };
			}

			// Build bridge
			if(this.boardAt(x,y) === world.tiles.water) {
				if(user.canBuildBridge()) {	
					this.setBoardAt(x,y,world.tiles.bridge);
					return { a: true, type: world.tiles.bridge };
				}
			}
		...
		Events: {
			...
			action: function(socket, pos) {
				...
				if(actionResult.a && actionResult.type === world.tiles.treeChopped) {  
					user.increaseWoodAmount();
					socket.emit('wood', {u: user});
					io.sockets.emit('worldChanged', {x: pos[0], y: pos[1], v: world.tiles.treeChopped});

					setTimeout(function() { 
						if(world.noneUserAction(pos[0], pos[1])) {
							io.sockets.emit('worldChanged', {x: pos[0], y: pos[1], v: world.tiles.tree});
						}
					}, 5000); 
				}

				if(actionResult.a && actionResult.type === world.tiles.bridge) {
					user.buildBridge();
					socket.emit('wood', {u: user});
					io.sockets.emit('worldChanged', {x: pos[0], y: pos[1], v: world.tiles.bridge});
				}  
			},
		},
	};
